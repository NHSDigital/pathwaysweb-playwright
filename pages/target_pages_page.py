...
class TargetPagesPage:
    ...
    def __init__(self, page: Page, party:str) -> None:
        self.page = page
        if party == "1st Party":
            expect(self.page.get_by_role("heading", name="Calling About Self")).to_be_visible()
        elif party == "3rd Party":
            expect(self.page.get_by_role("heading", name="Calling About Someone Else")).to_be_visible()
        else:
    ...
    def cx_care_advice_hc_triage(self) -> None:   
        self.page.get_by_role("button", name="a report of results or tests").click()
        self.page.get_by_text("Disposition considered inappropriate - alternative disposition required").click()
        ...
        self.page.get_by_role("button", name="Ok").click()
        expect(self.page.get_by_role("heading", name="Home care advice for: Social")).to_be_visible()
        self.page.get_by_label("Select a specific area for").select_option("Abdomen")
        self.page.get_by_label("Select a sub group for Abdomen area").select_option("10184849")
    ...
    def cx_care_advice_il_triage(self) -> None: 
        ...
        self.page.get_by_role("button", name="yes").click()
        self.page.get_by_role("button", name="yes").click()
        expect(self.main_content).to_contain_text("PA123.5900 Emergency Ambulance Response for Major Blood Loss (Category 2)")
        self.page.get_by_role("button", name=self.emergency_ambulance).click()
        self.page.get_by_role("button", name="no", exact=True).click()
        self.page.get_by_role("button", name="yes").click()
        self.page.get_by_role("button", name="no", exact=True).click()
        ...
        self.page.get_by_role("button", name="Next").click()
        self.page.get_by_role("button", name="no", exact=True).click()
        expect(self.page.locator("#main-content")).to_contain_text("PA25.4600 Closing instructions")
    ...
    def pa1_triage(self) -> None: 
        ...
        self.page.get_by_role("button", name="no", exact=True).click()
        self.page.get_by_role("button", name="the scene is safe", exact=True).click()
        expect(self.page.locator("#PartialStandard").get_by_text("PA1.0")).to_be_visible()
        self.page.get_by_role("button", name="yes").click()
        self.page.get_by_role("button", name="Early Exit").click()
        self.page.get_by_role("button", name="the phone line went dead").click()
        self.page.get_by_role("button", name="yes").click()
